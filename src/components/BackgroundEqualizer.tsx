...
// Optimized individual bar to prevent unnecessary re-rendering
    ...
    <AnimatePresence>
      {isPlaying && (
        ...
        >
          ...
          <div className="w-full flex items-end justify-center gap-1 sm:gap-1.5 md:gap-2 pt-3 pb-1 px-4 opacity-40 hover:opacity-75 transition-opacity">
            {barHeights.slice(0, 20).map((h, idx) => (
              <div
                key={`top-bar-${idx}`}
                className={`w-1 sm:w-1.5 md:w-2 rounded-full bg-gradient-to-b ${colorClasses.classicGradient} ${colorClasses.shadow} transition-[transform] duration-75 origin-top`}
                style={{
                  height: '24px',
                  transform: `scaleY(${Math.max(0.1, (h * 0.35) / 24)})`,
                  willChange: 'transform',
                }}
              />
            ))}
